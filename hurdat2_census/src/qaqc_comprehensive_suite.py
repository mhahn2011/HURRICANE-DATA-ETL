...
def create_wind_qaqc(storm_id: str = 'AL092021') -> folium.Map:
    ...
    envelope_geom, track_line, _ = create_storm_envelope(track, wind_threshold='64kt', alpha=0.6)
...
def main():
    ...
    # 1. Distance QA/QC
    print("\n[1/3] Generating Distance QA/QC visualization...")
    m1 = create_distance_qaqc()
    path1 = output_dir / "qaqc_01_distance.html"
    m1.save(str(path1))
    print(f"  ✓ Saved: {path1}")

    # 2. Wind QA/QC
    print("\n[2/3] Generating Max Wind QA/QC visualization...")
    m2 = create_wind_qaqc()
    path2 = output_dir / "qaqc_02_wind.html"
    m2.save(str(path2))
    print(f"  ✓ Saved: {path2}")

    # 3. Duration QA/QC
    print("\n[3/3] Generating Duration QA/QC visualization...")
    m3 = create_duration_qaqc()
    path3 = output_dir / "qaqc_03_duration.html"
    m3.save(str(path3))
    print(f"  ✓ Saved: {path3}")

    print("\n" + "=" * 70)
    print("QA/QC SUITE COMPLETE")
    print("=" * 70)
    ...
    print(f"  4. Wind Radii (existing):  qaqc_wind_radii_map.html")
    print("\nEach visualization validates algorithm correctness through:")
